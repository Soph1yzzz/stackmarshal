...
- User-local HMAC-signed checkpoints, repository-lineage identity, and resume inspection.
- Agent-neutral Core with the initial Codex adapter.
- Installable Codex Skill with bounded fallback scripts.
- Cross-platform CLI, JSON Schemas, 98% branch-aware test coverage, and adversarial tests.
- English primary README and Japanese secondary README.
- Reproducible release assets, symlink rejection, SHA-256 checksums, SBOM, and provenance metadata.
- Linux, macOS, Windows CI and CodeQL workflows with commit-pinned Actions.
- Fail-closed command approval, strict run IDs, and receipt-bound non-recursive rollback.
