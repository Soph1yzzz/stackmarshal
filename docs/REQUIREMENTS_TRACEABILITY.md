...
| State | Deterministic live transitions, one authoritative RUNNING run, user-local HMAC-authenticated live run state, nested-repository bootstrap lineage, phase snapshots, append-only events, pre-COMPLETE finalization, porcelain-safe terminal repository seal, and fail-closed workspace fingerprint diagnostics | `state.py`, `integrity.py`, `cli.py` | unit/integration, adversarial live-state tamper tests, `tests/test_nextpatch.py`, `tests/test_security_fixes.py`, and `tests/test_live_orchestration_contract.py` |
| Task graph | Canonical user-local HMAC-authenticated machine-readable task state, generated Markdown view, mandatory evidence gate before COMPLETE, finalization hash seal | `taskgraph.py`, `integrity.py`, `cli.py`, `schemas/task-graph.schema.json` | CLI/integration, adversarial task-state tamper tests, and live-orchestration contract tests |
...
| Security | Untrusted text, fail-closed command classes including Git helper/alias/global-option escapes, redaction, strict run IDs, authenticated live authority, workspace-state symlink/junction containment, release/installer boundaries | `security.py`, `integrity.py`, `state.py`, `taskgraph.py`, `cli.py`, `config.py`, release builder, verified installer, Skill policy | adversarial tests, focused source-backed release review, CodeQL, and risk-triggered Codex Security when justified |
| Installation | One-command bootstrap plus `pin latest`/exact pin/status/version workflow, verified pin bootstrap checksum, prerequisite approval, version pinning, isolated venv, atomic rollback, Skill backup, PATH consent, versioned restart-pending marker, stale-host fallback refusal, host/Skill/CLI readiness doctor, and non-executing authoritative-vs-shadowed launcher version diagnostics | `pinning.py`, `scripts/install.ps1`, `scripts/install.sh`, `scripts/installer.py`, `skills/stackmarshal/scripts/stackmarshal_core.py`, `cli.py` | `tests/test_pinning.py`, `tests/test_installer.py`, `tests/test_distribution.py`, `tests/test_nextpatch.py`, cross-platform installer smoke, `docs/INSTALLATION_AUDIT.md` |
