...
| State | Deterministic live transitions, one authoritative RUNNING run, nested-repository bootstrap lineage, phase snapshots, append-only events | `state.py`, `cli.py` | unit/integration and Nextpatch dogfood tests |
| Task graph | Canonical machine-readable task state, generated Markdown view, mandatory evidence gate before COMPLETE | `taskgraph.py`, `cli.py`, `schemas/task-graph.schema.json` | Nextpatch CLI/integration tests |
| Stop harness | Priority, live activity budgets, per-task attempts, failure fingerprints, stagnation | `harness.py`, `budget.py`, `activity.py`, `failure.py`, `progress.py` | stop/activity/Nextpatch tests |
