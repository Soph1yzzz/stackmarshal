...
| State | Deterministic live transitions, one authoritative RUNNING run, nested-repository bootstrap lineage, phase snapshots, append-only events, pre-COMPLETE finalization, porcelain-safe terminal repository seal, and fail-closed workspace fingerprint diagnostics | `state.py`, `cli.py` | unit/integration, `tests/test_nextpatch.py`, `tests/test_security_fixes.py`, and `tests/test_live_orchestration_contract.py` |
