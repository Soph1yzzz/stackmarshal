...
- Silent global mutation: explicit approval for missing Git/Python/venv installation, PATH changes, modified Skill replacement, and downgrade; shadowed launcher removal is a separate explicit `repair --remove-shadowed` action and only deletes launchers with high-confidence StackMarshal package evidence.
...
- Workspace escape/destructive writes or reads: strict run-id grammar, resolved-path containment, symlink/junction rejection across `.stackmarshal` state and `.gitignore` initialization, preexisting Git-ignore detection before mutating `.gitignore`, containment-aware workspace/terminal fingerprints that reject directory junction traversal before recursion, receipt-bound file rollback, and dirty-state capture.
...
- Forged live or resume state: canonical live `run.json` and `task-graph.json`, checkpoints, and acquisition receipts are authenticated with a user-local HMAC key kept outside the repository; unsigned or modified authority is rejected before use. Legacy unsigned state is archived byte-for-byte with SHA-256 evidence instead of being signed into authority. Resume additionally validates checkpoint HMAC, run id, repository-lineage identity, exact Git or non-Git worktree fingerprint, strict HEAD/dirty state, and the signed resume phase before reopening only explicitly resumable statuses.
