...
security classification, lock verification, checkpoint integrity, and CLI output.
...
`.stackmarshal/runs/<run-id>/`. Events are append-only JSONL. Architecture is frozen
